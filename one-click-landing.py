...
def navigate_to_calculated_setpoint(tracking_point_x, tracking_point_y, rangefinder_data):
    ...
    if math.hypot(abs(real_x), abs(real_y)) < 2.0:
        speed = 0.5
    elif math.hypot(abs(real_x), abs(real_y)) < 5:
        speed = 1.0
    else:
        speed = 3.0

    print("Navigate to x: ", real_x, " y: ", real_y,
          " z: ", rangefinder_data.range, "speed: ", speed)
    navigate(x=-real_y, y=-real_x, z=-rangefinder_data.range,
             yaw=float('nan'), speed=speed, frame_id='base_link', auto_arm=True)
    ...
    if tracker_initialized:
        ...
        if rospy.get_time() - last_time > 1 and telemetry.armed:
            ...
            if rangefinder_data.range > 2.0:
                navigate_to_calculated_setpoint(
...
def mavlink_message_handler(msg):
    ...
    if msg.msgid == 76:
        ...
        if mav_msg.command == 31011:
            print(mav_msg)
            if mav_msg.param1 != param1 and mav_msg.param2 != param2:
                run_tracker(640*mav_msg.param1, 480*mav_msg.param2)
                param1 = mav_msg.param1
                param2 = mav_msg.param2



mavlink_sub = rospy.Subscriber(
